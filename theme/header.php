...
<body <?php body_class(); ?>>
		...
		<div class="main-content-width h-[80px] lg:h-[120px] flex flex-row items-center">
			<div class="w-full flex flex-row justify-between items-center lg:justify-normal">
				...
				<a href="/" class="flex items-center"><span
						class="text-2xl lg:text-3xl font-cormorant uppercase tracking-wide">Cotidien</span></a>
					...
					<?php if (!is_cart()): ?>
						<button id="cart-toggle" class="relative flex items-center cursor-pointer">
							...
							<?php if (function_exists('WC') && WC()->cart->get_cart_contents_count() > 0): ?>
								<span
									class="absolute -top-2 -left-2 bg-black text-white text-[10px] leading-none font-medium px-[6px] py-[2px] rounded-full">
			...
			<?php if (!empty($cart)): ?>
				<div class="mt-4 w-full ">
					...
					<a href="<?php echo esc_url(wc_get_checkout_url()); ?>"
						class="w-full h-[50px] text-[18px] bg-black text-white text-center flex items-center justify-center  transition">
						Checkout
					</a>
				</div>
			<?php endif; ?>
	...
	<?php endif; ?>
	<nav id="mobile-nav"
		class="lg:hidden fixed top-0 right-0 w-full sm:w-[400px] h-full bg-white border-l-2 border-[#D9D9D9] transform translate-x-full transition-transform duration-300 ease-in-out z-50 p-6">
		<div class="w-full flex flex-row-reverse">
			<button id="menu-close" class="mb-6 self-end cursor-pointer">
				<svg class="w-8 h-8 mt-[8px] text-black" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
	...
	<div id="mobile-overlay"
		class="lg:hidden fixed inset-0 backdrop-blur-sm  bg-opacity-50 opacity-0 pointer-events-none z-40 transition-opacity duration-300">
	</div>
	<?php if (!is_cart()): ?>
